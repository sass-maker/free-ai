...
function processHistoryThrottles(history: ModelState['history'], acc: ProviderAccumulator): void {
...
function buildProviderStats(provider: string, acc: ProviderAccumulator): ProviderStats {
  const avgFirst =
...
function aggregateProviderStats(cache: Map<string, ModelState>, now: number): ProviderStats[] {
  ...
  for (const [key, state] of cache) {
    ...
    processHistoryThrottles(state.history, acc);
  ...
  for (const [provider, acc] of byProvider) {
    result.push(buildProviderStats(provider, acc));
  }
