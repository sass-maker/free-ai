...
video submit/poll routes from `src/routes/video-generation.ts`. Other
JSON/OpenAPI and provider-generation handlers remain in `index.ts` until their
own route groups have equivalent tests.
...
- Extraction started with HTML payloads, then moved the covered operator UI
  handlers, gateway auth middleware, and video route family behind focused tests.
