...
**Decision:** Split only route groups with focused coverage. Operator HTML
negotiation and the static dashboard, benchmark, model-catalog, and health UI
routes are now registered from `src/routes/operator-ui.ts`; JSON/OpenAPI and
provider-generation handlers remain in `index.ts` until their own route groups
...
- Extraction started with HTML payloads, then moved the covered operator UI
  handlers into `src/routes/operator-ui.ts`.
