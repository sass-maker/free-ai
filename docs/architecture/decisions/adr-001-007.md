...
image and video generation routes from dedicated modules under `src/routes/`.
Other JSON/OpenAPI and provider-generation handlers remain in `index.ts` until
their own route groups have equivalent tests.
...
- Extraction started with HTML payloads, then moved the covered operator UI
  handlers, gateway auth middleware, and image/video route families behind
  focused tests.
