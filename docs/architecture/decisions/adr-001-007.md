...
**Context:** Each gateway request needs the last-N attempt history, cooldown timestamps, and daily-usage counters for every model (80+ keys). This data is read on every inbound request and written after every provider call.
...
**Context:** As features were added (embeddings, multimodal, analytics, operator UI, auth middleware, budget guard), `index.ts` grew to ~3,645 lines (~55 KB) at the time of this decision and has since grown to ~4,000 lines (~112 KB). The file contains all Hono route handlers, middleware, and several inline helpers.

**Decision:** Split only groups with focused coverage. Operator HTML negotiation
and static UI routes are registered from `src/routes/operator-ui.ts`, while the
gateway auth middleware is registered from `src/middleware/gateway-auth.ts` and
embedding, image, video, and standalone TTS generation routes from dedicated
modules under `src/routes/`. Core chat/responses and uncovered route groups
remain in `index.ts` until their own route groups have equivalent tests.
...
- Extraction started with HTML payloads, then moved the covered operator UI
  handlers, gateway auth middleware, and embedding/image/video/TTS route families
...
**Tradeoffs:**
- Hard to navigate; adding a new route requires finding the right region in a 4,000-line file.
